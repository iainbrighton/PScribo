function OutWord 
{
  ...
  process {
    ...
    ## Create media part
    $documentUri = New-Object -TypeName System.Uri -ArgumentList ('/media', [System.UriKind]::Relative)
    WriteLog -Message ($localized.ProcessingDocumentPart -f $documentUri)
    $documentPart = $package.CreatePart($documentUri, 'application/x-directory')
    WriteLog -Message ($localized.WritingDocumentPart -f $documentUri)
